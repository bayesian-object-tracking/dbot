			       ...
			       std::vector<Eigen::MatrixXd>& data) const
{
    ...
    for (int part_index = 0; part_index < int(indices_.size()); part_index++)
    {
             ...
             triangle_index++)
        {
			...
			if((depth > 0.3 && depth < 6.0) &&
			   (data[5](row,col) == 0 || depth < data[5](row,col)) &&
			   (row > 0 && row < n_rows && col > 0 && col < n_cols)) {
			  data[0](row, col) = part_index;
			  data[1](row, col) = triangle_index;
			  data[2](row, col) = coord[0];
			  data[3](row, col) = coord[1];
			  data[4](row, col) = coord[2];
			  data[5](row, col) = depth;

			  data[6](row, col) =
			    trans_vertices[part_index][indices_[part_index][triangle_index][0]](0);
			  data[7](row, col) =
			    trans_vertices[part_index][indices_[part_index][triangle_index][0]](1);
			  data[8](row, col) =
			    trans_vertices[part_index][indices_[part_index][triangle_index][0]](2);
			  data[9](row, col) =
			    trans_vertices[part_index][indices_[part_index][triangle_index][1]](0);
			  data[10](row, col) =
			    trans_vertices[part_index][indices_[part_index][triangle_index][1]](1);
			  data[11](row, col) =
			    trans_vertices[part_index][indices_[part_index][triangle_index][1]](2);
			  data[12](row, col) =
			    trans_vertices[part_index][indices_[part_index][triangle_index][2]](0);
			  data[13](row, col) =
			    trans_vertices[part_index][indices_[part_index][triangle_index][2]](1);
			  data[14](row, col) =
			    trans_vertices[part_index][indices_[part_index][triangle_index][2]](2);
