...
template <typename T>
void Compare(const std::vector<T> vector_1,
             ...
             char* name_2)
{
    if (vector_1.size() == vector_2.size())
    {
        ...
        std::cout << "average difference between " << name_1 << " and "
                  << name_2 << " : " << difference / vector_1.size()
                  << std::endl;
    }
    else
    {
        std::cout << "WARNING: The two vectors to be compared have a different "
                     "amount of elements. "
                  << std::endl
                  << name_1 << ": " << vector_1.size() << ", " << name_2 << ": "
                  << vector_2.size() << std::endl
                  << "Comparison aborted." << std::endl;
    }
}

template <typename T>
void AverageDifference(const std::vector<T> vector_1,
                       const std::vector<T> vector_2,
                       char* name_1,
                       char* name_2)
{
...
template <typename T>
void Output(const T value, char* name)
{
    std::cout << name << ": " << value << std::endl;
}

template <typename T>
int IndexNextReal(const std::vector<T>& data,
                  const int& current_index = -1,
                  const int& step_size = 1)
{
    int index_next_real = current_index + step_size;
    while (index_next_real < int(data.size()) && index_next_real >= 0 &&
           !std::isfinite(data[index_next_real]))
        index_next_real += step_size;
...
public:
    ...
    ~Structurer2D() {}
    template <typename T>
    std::vector<T> Flatten(const std::vector<std::vector<T>>& deep_structure)
    {
    ...
    template <typename T>
    std::vector<std::vector<T>> Deepen(const std::vector<T>& flat_structure)
    {
...
public:
    ...
    std::vector<T> Flatten(
        const std::vector<std::vector<std::vector<T>>>& deep_structure)
    {
        size_t global_size = 0;
        sizes_.resize(deep_structure.size());
        for (size_t i = 0; i < deep_structure.size(); i++)
    ...
    template <typename T>
    std::vector<std::vector<std::vector<T>>> Deepen(
        const std::vector<T>& flat_structure)
    {
        size_t global_index = 0;
...
void LinearlyInterpolate(std::vector<T>& data)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        int index_first_real = IndexNextReal(
            data, limits[i] - step_direction[i], step_direction[i]);
        int index_next_real =
            IndexNextReal(data, index_first_real, step_direction[i]);
        if (index_next_real >= int(data.size()) || index_next_real < 0) return;

        double slope = double(data[index_next_real] - data[index_first_real]) /
    ...
    // interpolate
    int index_current_real =
        IndexNextReal(data, limits[0] - step_direction[0], step_direction[0]);
    int index_next_real =
        IndexNextReal(data, index_current_real, step_direction[0]);
    ...
    while (index_next_real < int(data.size()))
    {
        ...
        for (int i = index_current_real + 1; i < index_next_real; i++)
            data[i] =
                data[index_current_real] + (i - index_current_real) * slope;

        index_current_real = index_next_real;
        index_next_real =
            IndexNextReal(data, index_next_real, step_direction[0]);
    }
}
...
template <typename T>
void PrintVector(std::vector<std::vector<std::vector<T>>> v)
{
    for (size_t i = 0; i < v.size(); i++)
    {
        std::cout << i << " ================================" << std::endl;
...
Eigen::Matrix<T, -1, 1> Std2Eigen(const std::vector<T>& std)
{
    ...
    for (size_t i = 0; i < std.size(); i++) eigen(i) = std[i];
    return eigen;
}

template <typename Derived>
std::string Eigen2Mathematica(const Eigen::MatrixBase<Derived>& eigen,
                              const std::string& name = "")
{
    std::string mathematica;
    if (!name.empty()) mathematica += name + " = ";
    ...
    for (int row = 0; row < eigen.rows(); row++)
    {
        ...
        for (int col = 0; col < eigen.cols(); col++)
        {
            ...
            if (col != eigen.cols() - 1) mathematica += ", ";
        }
        mathematica += "}";
        if (row != eigen.rows() - 1) mathematica += ", ";
    }
    mathematica += "}";
    if (!name.empty()) mathematica += ";";

    return mathematica;
}

// returns the first index where the two vectors differ, if there is no
// difference then -1 is returned.
template <typename T, int n_rows, int n_cols>
int DifferenceAt(const std::vector<Eigen::Matrix<T, n_rows, n_cols>>& a,
        ...
        for (size_t row = 0; row < n_rows; row++)
            for (size_t col = 0; col < n_cols; col++)
            {
                if (isnan(a[i](row, col)) && isnan(b[i](row, col)))
                    ...
                    return i;
                else if ((a[i](row, col) - b[i](row, col) > epsilon) ||
                         (b[i](row, col) - a[i](row, col) > epsilon))
                    return i;
            }
...
// difference then -1 is returned.
template <typename T>
int DifferenceAt(const std::vector<T>& a,
...
template <typename T>
std::vector<T> ReverseOrder(std::vector<T> ordered)
{
...
template <typename T>
std::vector<T> Count(T from, T to, T increment = 1)
{
...
struct ValuesIndex
{
    ...
    bool operator<(const ValuesIndex& right_side) const
    {
        ...
        {
            if (values[i] < right_side.values[i]) return true;
            if (values[i] > right_side.values[i]) return false;
        }
        return false;
    }
...
template <typename T>
void SortAndCollapse(std::vector<std::vector<T>>& values,
                     std::vector<size_t>& multiplicities)
{
    ...
    {
        if (i > 0 &&
            temp_values[values_indices[i - 1].index] !=
                temp_values[values_indices[i].index])
...
struct ValueIndex
{
    ...
    bool operator<(const ValueIndex& str) const { return (value < str.value); }
};
template <typename T>
std::vector<int> SortAscend(const std::vector<T>& values)
{
...
template <typename T>
std::vector<int> SortDescend(const std::vector<T>& values)
{
...
template <typename T>
void Sort(std::vector<T>& ref_values, bool order = 0)
{
...
void SortAscend(std::vector<T0>& ref_values, std::vector<T1>& values)
{
...
template <typename T0, typename T1>
void SortDescend(std::vector<T0>& ref_values, std::vector<T1>& values)
{
    std::vector<T0> temp_ref_values = ref_values;
    std::vector<T1> temp_values = values;
...
void SortAscend(std::vector<T0>& ref_values,
...
template <typename T0, typename T1, typename T2>
void SortDescend(std::vector<T0>& ref_values,
                 std::vector<T1>& values1,
                 std::vector<T2>& values2)
{
    std::vector<T0> temp_ref_values = ref_values;
...
template <typename T>
int BoundIndex(const std::vector<T>& values,
               bool bound_type)  // bound type 1 for max and 0 for min
{
    int BoundIndex = 0;
    T bound_value = bound_type ? -std::numeric_limits<T>::max()
...
template <typename T>
T bound_value(const std::vector<T>& values,
              bool bound_type)  // bound type 1 for max and 0 for min
{
...
std::vector<Tout> Apply(const std::vector<Tin>& input, Tout (*f)(Tin))
{
    ...
    for (size_t i = 0; i < output.size(); i++) output[i] = (*f)(input[i]);

    return output;
}

template <typename T>
std::vector<T> SetSum(const std::vector<T>& input, T sum)
{
    T old_sum = 0;
    for (size_t i = 0; i < input.size(); i++) old_sum += input[i];
    T factor = sum / old_sum;

    std::vector<T> output(input.size());
    for (size_t i = 0; i < input.size(); i++) output[i] = factor * input[i];
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
template <typename T>
Eigen::Quaternion<typename T::Scalar> Delta2Quaternion(
    const Eigen::MatrixBase<T>& delta)
{
...
// this function finds the intersection between two lines. a is a point on line
// and d_a is the direction
// vector of the line. if lines do not cross, this will return the point which
// minimizes the squared
// distance between the two lines. if they are parallel, then inf will be
// returned.
    ...
    const Eigen::MatrixBase<T4>& d_b_in)
{
    ...
    double c_a =
        d_a.dot((b - a) - d_b.dot(b - a) * d_b) / (1. - pow(d_b.dot(d_a), 2));
    double c_b =
        d_b.dot((a - b) - d_a.dot(a - b) * d_a) / (1. - pow(d_a.dot(d_b), 2));

    return ((b + c_b * d_b) + (a + c_a * d_a)) / 2.;
}

template <typename T1, typename T2>
typename T1::Scalar Angle(const Eigen::MatrixBase<T1>& a,
...
template <typename T>
Eigen::Matrix<typename T::Scalar, 3, 1> OrthogonalUnitVector(
    const Eigen::MatrixBase<T>& v)
{
    Eigen::Matrix<typename T::Scalar, 3, 1> n =
        (v.cross(Eigen::Matrix<typename T::Scalar, 3, 1>(1.0, 0, 0)))
            .normalized();
    if (!std::isfinite(n.norm()))
        n = (v.cross(Eigen::Matrix<typename T::Scalar, 3, 1>(0, 1.0, 0)))
                .normalized();
...
template <typename T1, typename T2>
Eigen::AngleAxis<typename T1::Scalar> AngleAxis(const Eigen::MatrixBase<T1>& a,
                                                const Eigen::MatrixBase<T2>& b)
{
    ...
    // if the axis is inf then the angle is either 0 or pi, so the axis has to
    // be just some orthogonal vector
    if (!std::isfinite(axis.norm())) axis = OrthogonalUnitVector(a);
...
public:
    ...
    void PreRotate(
        ...
        const Eigen::Matrix<T, 3, 1>& c = Eigen::Matrix<T, 3, 1>::Zero())
    {
        t_ = t_ - R_ * R * c + R_ * c;
        R_ = R_ * R;
    }
    void PreTranslate(const Eigen::Matrix<T, 3, 1>& t) { t_ = t_ + R_ * t; }
    void PostRotate(
        const Eigen::Matrix<T, 3, 3>& R,
        const Eigen::Matrix<T, 3, 1>& c = Eigen::Matrix<T, 3, 1>::Zero())
    {
    ...
    void PostTranslate(const Eigen::Matrix<T, 3, 1>& t) { t_ = t_ + t; }
...
template <typename T>
Eigen::Matrix<T, 2, 1> CartCoord2ImageCoord(
...
template <typename T>
Eigen::Matrix<int, 2, 1> CartCoord2ImageIndex(
    const Eigen::Matrix<T, 3, 1>& cart,
    const Eigen::Matrix<T, 3, 3>& camera_matrix)
{
...
template <typename T>
Eigen::Matrix<T, 3, 1> ImageCoord2CartCoord(
...
// coordinates
template <typename T>
Eigen::Matrix<T, 3, 1> ImageIndex2CartCoord(
...
template <typename T>
Eigen::Matrix<Eigen::Matrix<T, 3, 1>, -1, -1> Image2Points(
    const Eigen::Matrix<T, -1, -1>& image,
    const Eigen::Matrix<T, 3, 3>& camera_matrix)
{
    Eigen::Matrix<T, 3, 3> camera_matrix_inverse = camera_matrix.inverse();

    Eigen::Matrix<Eigen::Matrix<T, 3, 1>, -1, -1> points(image.rows(),
                                                         image.cols());
...
template <typename T>
std::vector<Eigen::Matrix<T, 3, 1>> DepthImage2CartVectors(
    ...
    for (size_t row = 0; row < n_rows; row++)
        for (size_t col = 0; col < n_cols; col++)
            vectors[row * n_cols + col] =
                ImageIndex2CartCoord(Eigen::Vector2i(row, col),
...
template <typename T>
std::vector<T> CartVectors2DepthImage(
    ...
    const Eigen::Matrix<T, 3, 3>& camera_matrix)
{
    ...
    {
        if (isnan(vectors[i](0)) || isnan(vectors[i](1)) ||
            isnan(vectors[i](2)))
            continue;

        Eigen::Matrix<int, 2, 1> index =
            CartCoord2ImageIndex(vectors[i], camera_matrix);

        if (index(0) < 0 || index(0) >= n_rows || index(1) < 0 ||
            index(1) >= n_cols)
        {
            std::cout << "ERROR: in CartVectors2DepthImage vector comes to lie "
                         "outside of image "
                      << std::endl;
            exit(-1);
        }
...
inline double log_sum_exp(double a, double b, double c)
{
    ...
    if (b > c && b > a)
        return b + log(1 + exp(a - b) + exp(c - b));
    else
        return c + log(1 + exp(a - c) + exp(b - c));
}
inline double log_sum_exp(double a, double b, double c, double d)
{
    ...
    if (b > a && b > c && b > d)
        return b + log(1 + exp(a - b) + exp(c - b) + exp(d - b));
    if (c > a && c > b && c > d)
        return c + log(1 + exp(a - c) + exp(b - c) + exp(d - c));
    else
        return d + log(1 + exp(a - d) + exp(b - d) + exp(c - d));
}

template <typename T>
T log_sum_exp(std::vector<T> exponents)
{
    T max_exponent = -std::numeric_limits<double>::max();
    for (size_t i = 0; i < exponents.size(); i++)
        if (exponents[i] > max_exponent) max_exponent = exponents[i];

    for (size_t i = 0; i < exponents.size(); i++) exponents[i] -= max_exponent;

    T sum = 0;
    for (size_t i = 0; i < exponents.size(); i++) sum += exp(exponents[i]);
...
public:
    LogSumExp() {}
    ~LogSumExp() {}
    void add_exponent(T exponent) { exponents_.push_back(exponent); }
    T Compute()
    {
...
public:
    template <typename T>
    DiscreteSampler(std::vector<T> log_prob)
    {
        ...
        for (int i = 0; i < int(log_prob.size()); i++) log_prob[i] -= max;
        ...
        for (int i = 0; i < int(prob.size()); i++) prob[i] /= sum;
        ...
        for (int i = 1; i < int(log_prob.size()); i++)
            cumulative_prob_[i] =
                cumulative_prob_[i - 1] + prob[sorted_indices_[i]];
    }
    ...
    int MapStandardGaussian(double gaussian_sample) const
    {
        ...
        int sample_index = 0;
        while (uniform_sample > cumulative_prob_[sample_index])
            sample_index++;  // TODO: THIS COULD BE DONE IN LOG TIME
...
public:
    template <typename T>
    DiscreteDistribution(std::vector<T> log_prob)
    {
    ...
    {
        Eigen::Matrix<T, 2, 1> image =
            CartCoord2ImageCoord(cart, camera_matrix);
        Eigen::Matrix<int, 2, 1> image_index;
        image_index(0) = floor(image(1) + 0.5);
        image_index(1) = floor(image(0) + 0.5);
    ...
    // converts from image coordinates and depth (z value) to cartesian
    // coordinates
    template <typename T>
    Eigen::Matrix<T, 3, 1> ImageCoord2CartCoord(
        const Eigen::Matrix<T, 2, 1>& image,
        ...
        const Eigen::Matrix<T, 3, 3>& camera_matrix)
    {
        ...
        for (size_t i = 0; i < vectors.size(); i++)
        {
            ...
            if (index(0) < 0 || index(0) >= n_rows || index(1) < 0 ||
                index(1) >= n_cols)
            {
                std::cout << "ERROR: in CartVectors2DepthImage vector comes to "
                             "lie outside of image "
                          << std::endl;
                exit(-1);
            }
    ...
    int Sample() { return MapStandardUniform(uniform_sampler_()); }
    int MapStandardGaussian(double gaussian_sample) const
    {
        double uniform_sample =
            0.5 * (1.0 + std::erf(gaussian_sample / std::sqrt(2.0)));
    ...
    {
        std::vector<double>::const_iterator iterator = std::lower_bound(
            cumulative_prob_.begin(), cumulative_prob_.end(), uniform_sample);
...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
template <typename T>
void Vector2TranslAndQuat(const std::vector<T>& v,
...
template <typename T>
void TranslAndQuat2Vector(const Eigen::Quaternion<T>& q,
...
template <typename T>
void Vector2TranslAndRot(const std::vector<T>& v,
...
template <typename T>
void TranslAndRot2Vector(const Eigen::Matrix<T, 3, 3>& R,
                         const Eigen::Matrix<T, 3, 1>& t,
                         std::vector<T>& v)
{
...
{
    Eigen::VectorXd new_state(old_state.size() * 2 -
                              1);  // the new state also contains the velocity
    ...
    //	new_state.topRows(3) +=
    //Eigen::Quaterniond(new_state.middleRows<4>(3))._transformVector(center);
...
template <typename T>
void Vector2QuatAndTransl(const std::vector<T>& v,
                          Eigen::Quaternion<T>& q,
                          Eigen::Matrix<T, 3, 1>& t)
{
    q.w() = v[0];
...
template <typename T>
void QuatAndTransl2Vector(const Eigen::Quaternion<T>& q,
                          const Eigen::Matrix<T, 3, 1>& t,
                          std::vector<T>& v)
{
    v.resize(7);
...
template <typename T>
void Vector2RotAndTransl(const std::vector<T>& v,
                         Eigen::Matrix<T, 3, 3>& R,
                         Eigen::Matrix<T, 3, 1>& t)
{
    Eigen::Quaternion<T> q;
...
template <typename T>
void RotAndTransl2Vector(const Eigen::Matrix<T, 3, 3>& R,
                         const Eigen::Matrix<T, 3, 1>& t,
                         std::vector<T>& v)
{
    const Eigen::Quaternion<T> q(R);
...
template <typename T>
void Vector2Hom(const std::vector<T>& v, Eigen::Matrix<T, 4, 4>& H)
{
    Eigen::Matrix<T, 3, 3> R;
    Eigen::Matrix<T, 3, 1> t;
...
template <typename T>
void Hom2Vector(const Eigen::Matrix<T, 4, 4>& H, std::vector<T>& v)
{
...
template <typename T>
void Vector2Affine(const std::vector<T>& v,
...
template <typename T>
void Affine2Vector(const Eigen::Transform<T, 3, Eigen::Affine>& A,
                   std::vector<T>& v)
{
    Eigen::Matrix<T, 4, 4> H;
    H = A.matrix();
